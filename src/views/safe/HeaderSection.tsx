...
export default function HeaderSection({ safeHash, transactionHash, network, loading }: any) {
    return (
        <div>
            <section className="px-3 ">
                <div className="container  bg-white rounded shadow-200 px-[16px] py-[12px]">
                    <h3 className="text-[15px]  leading-[28px] font-bold text-dark-600">Safe Transaction ID</h3>
                    {loading ? (
                        <Skeleton height={55} />
                    ) : (
                        <div className="md:flex items-center gap-4 pt-[14px] pb-[2px]">
                            ...
                            <div className="flex items-center flex-1 gap-2 break-words">
                                <span className="text-sm leading-5 break-all text-dark-600">{safeHash}</span>
                                <CopyButton text={transactionHash} />
                                <button className="outline-none focus:outline-none ring-0 focus:ring-0">
                                    <Link
                                        href={`${NETWORK_SCANNER_MAP[network]}/tx/${transactionHash}`}
                                        aria-current="page"
